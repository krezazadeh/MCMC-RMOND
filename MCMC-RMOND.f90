...
!         mpiifort -O2 MCMC-RMOND.f90 -o MCMC-RMOND
!   - Run with desired number of parallel chains:
!         mpirun -np <nchains> ./MCMC-RMOND
!
! NOTES:
...
program MCMC
    ...
    ! MCMC
    integer, parameter :: max_try = 1000
...
!     sigma0 = 3.46667*1.0d8
!     Rd = 9.37637d0
!     r0 = 16.0d0

    initpoint = (/ 3.78572d0, 8.09422d0, 0.0461075d0, 25.2847d0 /)
    priormin = (/ 0.0d0, 0.0d0, -0.5d0, 10.0d0 /)
    priormax = (/ 10.0d0, 20.0d0, 0.5d0, 40.0d0 /)
    jumpsize = (/ 0.01d0, 0.01d0, 0.001d0, 0.01d0 /)

!     initpoint = (/ 0.0324294d0 /)
!     priormin = (/ 0.001d0 /)
!     priormax = (/ 0.5d0 /)
!     jumpsize = (/ 0.001d0 /)
...
!     stop
    ...
    do i = 1, max_iter
        ...
        Rd = params_new(2)
        params_new(3) = 0.0461075d0
        epsilon = params_new(3)
        params_new(4) = 25.2847d0
        r0 = params_new(4)
        ...
        , Md(dataNGC3198(ndataNGC3198, 1)), absCA(), a0(), rDebye(), aDebye()
...
function vRMOND(r)
    ...
    if (r <= r0) then
        ...
        vRMOND = (sqrt(absCA())*c**(1.0d0 + (3.0d0*epsilon)/4.0d0)* &
        (kpc*r)**(epsilon/2.0d0)*sqrt(abs(epsilon)))/ &
        (1.0d3*sqrt(2.0d0)*G**(epsilon/4.0d0)* &
        hbar**(epsilon/4.0d0))
    end if
...
function absCA()
    ...
    absCA = (2.0d6*c**(-2.0d0 - (3.0d0*epsilon)/2.0d0)* &
    G**(epsilon/2.0d0)*Gt*hbar**(epsilon/2.0d0)* &
    r0**(-1.0d0 - epsilon)*Md(r0))/ &
    (kpc**epsilon*abs(epsilon))
